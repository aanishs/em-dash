...
console.log(`  In tool-bindings: ${allMappedControls.size - uniqueUnmapped.length}`);
...
if (!ig) {
  ...
  const missingFromIg = [...mappingSafeguards].filter(s => !allIgSafeguards.has(s));
  ...
  const orphanedIg = [...allIgSafeguards].filter(s => !mappingSafeguards.has(s));
