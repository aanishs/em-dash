...
for (const dir of skillDirs) {
	...
	if (!content.includes("AUTO-GENERATED"))
		issues.push("missing AUTO-GENERATED header");
	...
	if (!content.startsWith("---")) {
		...
		if (fmEnd > 3) {
			...
			if (!fm.includes("version:"))
				issues.push("missing version: in frontmatter");
			if (!fm.includes("allowed-tools:"))
				issues.push("missing allowed-tools: in frontmatter");
		}
	}
...
for (const policy of EXPECTED_REGO) {
	...
	if (!/^package hipaa\.\w+/m.test(content))
		issues.push("missing package hipaa.* declaration");
	if (!content.includes("deny[msg]")) issues.push("missing deny[msg] rule");
	if (!content.includes("hipaa_ref"))
		issues.push("missing hipaa_ref in deny messages");
