...
const byTier: Record<string, { runs: number; tests: number; passed: number; cost: number }> = {};

for (const file of files) {
  try {
    const data: EvalResult = JSON.parse(fs.readFileSync(path.join(evalDir, file), 'utf-8'));
    totalRuns++;
...
console.log(`  Total passed:   ${totalPassed} (${totalTests > 0 ? Math.round(totalPassed / totalTests * 100) : 0}%)`);
console.log(`  Total cost:     $${totalCost.toFixed(2)}`);
console.log(`  Total duration: ${Math.round(totalDuration / 1000)}s`);
console.log(`  Avg cost/run:   $${totalRuns > 0 ? (totalCost / totalRuns).toFixed(2) : '0.00'}`);

console.log('\n  By tier:');
for (const [tier, stats] of Object.entries(byTier)) {
  const passRate = stats.tests > 0 ? Math.round(stats.passed / stats.tests * 100) : 0;
  console.log(`    ${tier}: ${stats.runs} runs, ${stats.tests} tests (${passRate}% pass), $${stats.cost.toFixed(2)}`);
}
