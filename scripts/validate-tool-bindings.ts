...
function findProwlerComplianceDir(): string | null {
	...
	// Try pip show
	try {
		const pipOut = execSync("pip show prowler 2>/dev/null", {
			encoding: "utf-8",
		});
...
function loadProwlerCIS(complianceDir: string): {
	...
	const data = JSON.parse(
		fs.readFileSync(path.join(complianceDir, cisFile), "utf-8"),
	);

	console.log(
		`Prowler CIS source: ${cisFile} (${data.Requirements?.length || 0} requirements)`,
	);
...
if (!prowlerDir) {
	...
	console.error(
		"Or specify: bun scripts/validate-tool-bindings.ts --prowler-path /path/to/compliance/aws",
	);
	process.exit(1);
}
...
for (const [controlId, binding] of Object.entries(bindings.bindings) as [
	string,
	{
		prowler?: string[];
	...
	for (const checkId of binding.prowler || []) {
		...
		if (allChecks.has(checkId)) {
			...
			console.log(
				`  ✗ ${controlId} → ${checkId} — NOT FOUND in Prowler CIS data`,
			);
			warnings++;
		}
	}
...
console.log(
	`  ${verified} verified, ${warnings} not found in CIS data (may be HIPAA/PCI-only checks)`,
);
console.log();
...
for (const [controlId, binding] of Object.entries(bindings.bindings) as [
	...
	for (const cisId of cisBenchmarks) {
		...
		if (prowlerForCIS.length === 0) {
			console.log(
				`  ? ${controlId} cis_benchmark ${cisId} — no Prowler checks found for this CIS ID`,
			);
			warnings++;
			continue;
		}
		...
		if (overlap.length > 0) {
			...
			const missing = prowlerForCIS.filter(
				(p: string) => !ourProwler.includes(p),
			);
			if (missing.length > 0) {
				console.log(
					`  ✗ ${controlId} cis_benchmark ${cisId} — Prowler maps these checks but we don't have them:`,
				);
...
if (missingChecks.length > 0) {
	console.log(
		`  ${missingChecks.length} Prowler CIS checks not in our bindings:`,
	);
	for (const { check, cisIds } of missingChecks.slice(0, 20)) {
		console.log(`    ${check} (CIS ${cisIds.join(", ")})`);
	}
	if (missingChecks.length > 20)
		console.log(`    ... and ${missingChecks.length - 20} more`);
} else {
...
for (const [controlId, tools] of Object.entries(bindings.bindings) as [
	string,
	{
		prowler?: string[];
		cis_benchmark?: string[];
	...
	for (const id of tools.checkov || []) {
		if (/^CKV_[A-Z]+_\d+$/.test(id)) {
			...
			checkovInvalid++;
			console.log(
				`  INVALID Checkov ID: ${id} in ${controlId} (expected CKV_[A-Z]+_[0-9]+)`,
			);
		}
	}
	for (const id of tools.trivy || []) {
		if (/^AVD-[A-Z]+-\d{4}$/.test(id)) {
			...
			trivyInvalid++;
			console.log(
				`  INVALID Trivy ID: ${id} in ${controlId} (expected AVD-[A-Z]+-[0-9]{4})`,
			);
		}
	}
}
...
if (totalIssues === 0 && warnings === 0) {
	...
	console.log(
		`PASS with ${warnings} warning(s) — some checks may be framework-specific (not CIS)`,
	);
} else {
	console.log(
		`ISSUES FOUND: ${cisMismatch} CIS mismatch(es), ${checkovInvalid} invalid Checkov ID(s), ${trivyInvalid} invalid Trivy ID(s)`,
	);
}
