...
let files: string[];
try {
  files = fs.readdirSync(evalDir).filter(f => f.endsWith('.json') && !f.startsWith('_'));
} catch {
...
console.log(`  ${'File'.padEnd(55)} ${'Tests'.padStart(6)} ${'Pass'.padStart(5)} ${'Cost'.padStart(7)} ${'Duration'.padStart(10)}`);
...
for (const file of files) {
  try {
    ...
    console.log(`  ${name.padEnd(55)} ${tests.padStart(6)} ${passed.padStart(5)} ${cost.padStart(7)} ${dur.padStart(10)}`);
  } catch {
