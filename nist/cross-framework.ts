...
function loadAllFilters(): Map<string, Record<string, string[]>> {
  ...
  for (const file of fs.readdirSync(NIST_DIR)) {
    ...
    try {
      const data = JSON.parse(fs.readFileSync(path.join(NIST_DIR, file), 'utf-8'));
...
function loadCisBenchmarkRefs(): Map<string, string[]> {
  ...
  for (const [controlId, binding] of Object.entries(data.bindings) as [string, any][]) {
    if (binding.cis_benchmark?.length > 0) {
...
export function buildCrossFrameworkMatrix(activeFrameworks?: string[]): CrossFrameworkMatrix {
  ...
  const controls: ControlFrameworkEntry[] = [...controlToFrameworks.entries()]
    ...
    .sort((a, b) => b.framework_count - a.framework_count || a.control_id.localeCompare(b.control_id));
...
export function formatMatrixTable(matrix: CrossFrameworkMatrix): string {
  ...
  const header = ['Control', ...fws.map(f => f.toUpperCase().padEnd(8)), 'Impact'].join('  ');
  ...
  for (const entry of matrix.controls) {
    const cols = fws.map(f => entry.frameworks.includes(f) ? '  ✓     ' : '  ·     ');
    const impact = `${entry.framework_count}/${fws.length}`;
    lines.push([entry.control_id.padEnd(9), ...cols, impact].join(''));
  }

  lines.push(sep);
  lines.push(`${matrix.total_controls} controls across ${fws.length} frameworks`);
