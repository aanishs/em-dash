...
function getFailures(
	...
	namespace?: string,
): NonNullable<ConftestResult["failures"]> {
...
function _totalFailures(results: ConftestResult[]): number {
...
describe("AWS encryption at rest", () => {
	test("denies unencrypted RDS, S3, EBS, KMS, SNS, SQS", () => {
		...
		const results = runConftest(
			path.join(FIXTURES_DIR, "bad-aws-encryption.json"),
		);
		...
		for (const f of failures) {
			expect([
				"rego-aws-s3-encryption",
				"rego-aws-ebs-encryption",
				"rego-aws-rds-encryption",
				"rego-aws-kms-rotation",
				"rego-aws-sns-encryption",
				"rego-aws-sqs-encryption",
			]).toContain(f.metadata.check_id);
		}
	});
...
describe("AWS backup and disaster recovery", () => {
	test("denies disabled backups and retention below 35 days", () => {
		...
		const messages = failures.map((f) => f.msg);
		expect(messages.some((m) => m.includes("automated backups disabled"))).toBe(
			true,
		);
...
describe("AWS transmission security", () => {
	test("denies open SG, HTTP listener, public RDS", () => {
		if (skipIfNoConftest()) return;
		const results = runConftest(
			path.join(FIXTURES_DIR, "bad-aws-transmission.json"),
		);
		...
		expect(
			messages.some(
				(m) => m.includes("Security group rule") && m.includes("port 22"),
			),
		).toBe(true);
		expect(
			messages.some(
				(m) => m.includes("Security group") && m.includes("port 5432"),
			),
		).toBe(true);
		expect(
			messages.some(
				(m) => m.includes("Load balancer listener") && m.includes("HTTP"),
			),
		).toBe(true);
		...
		for (const f of failures) {
			expect([
				"rego-aws-sg-rule-open",
				"rego-aws-sg-open",
				"rego-aws-lb-https",
				"rego-aws-rds-sg-open",
				"rego-rds-public",
			]).toContain(f.metadata.check_id);
		}
	});
...
describe("AWS access control", () => {
	test("denies wildcard IAM policies and missing MFA", () => {
		...
		expect(
			messages.some(
				(m) => m.includes("admin_string") && m.includes("wildcard"),
			),
		).toBe(true);
		expect(
			messages.some(
				(m) => m.includes("admin_array") && m.includes("array form"),
			),
		).toBe(true);
...
describe("AWS audit logging", () => {
	test("denies misconfigured CloudTrail, VPC, CloudWatch", () => {
		...
		for (const f of failures) {
			expect([
				"rego-aws-cloudtrail-multiregion",
				"rego-aws-cloudtrail-logvalidation",
				"rego-aws-cloudtrail-encryption",
				"rego-aws-vpc-flowlogs",
				"rego-aws-cloudwatch-retention",
				"rego-aws-cloudwatch-no-retention",
			]).toContain(f.metadata.check_id);
		}
	});
...
describe("AWS secrets", () => {
	test("denies hardcoded keys and default passwords", () => {
		if (skipIfNoConftest()) return;
		const results = runConftest(
			path.join(FIXTURES_DIR, "bad-aws-secrets.json"),
		);
...
describe("GCP controls", () => {
	test("denies insecure Cloud SQL, GCS, firewall, IAM, audit logging", () => {
		...
		const transFailures = getFailures(
			results,
			"compliance.transmission_security",
		);
...
describe("Azure controls", () => {
	test("denies subscription-scope Contributor", () => {
		...
		expect(failures[0].metadata.check_id).toBe(
			"rego-azure-role-subscription-scope",
		);
	});
...
describe("Kubernetes security", () => {
	test("denies root, privileged, unapproved registry, inline secrets", () => {
		if (skipIfNoConftest()) return;
		const results = runConftest(
			path.join(FIXTURES_DIR, "bad-k8s-deployment.yaml"),
		);
		...
		expect(
			messages.some(
				(m) => m.includes("DB_PASSWORD") && m.includes("inline value"),
			),
		).toBe(true);
	...
	test("denies wildcard ClusterRole", () => {
		if (skipIfNoConftest()) return;
		const results = runConftest(
			path.join(FIXTURES_DIR, "bad-k8s-clusterrole.yaml"),
		);
	...
	test("denies PHI namespace without NetworkPolicy", () => {
		if (skipIfNoConftest()) return;
		const results = runConftest(
			path.join(FIXTURES_DIR, "bad-k8s-namespace.yaml"),
		);
		const failures = getFailures(results, "compliance.k8s_security");
		expect(failures.length).toBe(1);
	...
	test("denies privileged StatefulSet", () => {
		if (skipIfNoConftest()) return;
		const results = runConftest(
			path.join(FIXTURES_DIR, "bad-k8s-statefulset.yaml"),
		);
		const failures = getFailures(results, "compliance.k8s_security");
		expect(failures.length).toBe(1);
	...
	test("passes compliant K8s deployment", () => {
		if (skipIfNoConftest()) return;
		const results = runConftest(
			path.join(FIXTURES_DIR, "good-k8s-deployment.yaml"),
		);
		const failures = getFailures(results);
		expect(failures.length).toBe(0);
	});
...
describe("Cross-policy coverage", () => {
	...
	test("all check_ids are valid registry entries", () => {
		...
		const validCheckIds = new Set([
			...
			"rego-aws-kms-rotation",
			"rego-aws-sns-encryption",
			"rego-gcp-cloudsql-encryption",
			"rego-gcp-gcs-encryption",
			"rego-aws-sqs-encryption",
			"rego-azure-storage-https",
			"rego-azure-disk-encryption",
			"rego-azure-keyvault-softdelete",
			"rego-gcp-bigquery-encryption",
			// backup-dr.rego
			"rego-aws-rds-backup",
			"rego-aws-rds-backup-retention",
			"rego-aws-s3-versioning",
			"rego-aws-dynamodb-backup",
			"rego-aws-efs-backup",
			"rego-gcp-cloudsql-backup",
			"rego-azure-sql-backup",
			// transmission-security.rego
			"rego-aws-sg-rule-open",
			"rego-aws-sg-open",
			"rego-aws-lb-https",
			"rego-aws-rds-sg-open",
			"rego-gcp-sql-authorized-networks",
			"rego-gcp-sql-public-ip",
			"rego-gcp-firewall-open",
			"rego-azure-nsg-open",
			"rego-azure-appservice-https",
			"rego-gcp-cloudrun-public",
			// audit-logging.rego
			"rego-aws-cloudtrail-multiregion",
			"rego-aws-cloudtrail-logvalidation",
			"rego-aws-cloudtrail-encryption",
			...
			"rego-aws-cloudwatch-retention",
			"rego-aws-cloudwatch-no-retention",
			"rego-gcp-audit-logging",
			"rego-azure-monitor-logprofile",
			"rego-azure-monitor-diagnostic",
			"rego-gcp-logging-sink",
			// access-control.rego
			"rego-aws-iam-wildcard-action",
			"rego-aws-iam-wildcard-resource",
			"rego-aws-iam-admin-policy",
			"rego-gcp-iam-primitive-role",
			"rego-gcp-iam-alluser-binding",
			"rego-azure-role-subscription-scope",
			"rego-azure-role-wildcard-scope",
			"rego-gcp-sa-key-rotation",
			"rego-gcp-iam-allauthenticated",
			"rego-azure-role-wildcard-actions",
			// secrets.rego
			"rego-aws-provider-hardcoded-key",
			"rego-aws-provider-hardcoded-secret",
			"rego-tf-sensitive-default",
			"rego-tf-secret-var-default",
			"rego-k8s-secret-in-env",
			// container-security.rego
			"rego-docker-nonroot-user",
			"rego-docker-latest-tag",
			"rego-docker-add-url",
			"rego-docker-expose-all",
			"rego-compose-readonly",
			// k8s-security.rego
			"rego-k8s-deploy-nonroot",
			"rego-k8s-deploy-seccontext",
			"rego-k8s-deploy-nonprivileged",
			"rego-k8s-namespace-netpolicy",
			"rego-k8s-deploy-approved-registry",
			"rego-k8s-deploy-secret-env",
			"rego-k8s-statefulset-nonroot",
		]);
