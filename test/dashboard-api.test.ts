...
interface ComplianceSummaryResponse {
	controls: Array<{ oscal_id: string; [key: string]: unknown }>;
	summary: { total: number; [key: string]: unknown };
}

interface ComplianceControlResponse {
	control: { oscal_id: string; [key: string]: unknown };
	checks: unknown[];
}
...
interface FindingsResponse {
	findings: unknown[];
	filter: { result: string; limit: number; [key: string]: unknown };
}
...
beforeAll(async () => {
	...
	serverProc = Bun.spawn(
		[
			"bun",
			path.join(ROOT, "scripts", "dashboard-server.ts"),
			"--port",
			String(port),
			"--project-dir",
			tmpDir,
...
describe("GET /api/compliance", () => {
	test("?view=summary returns controls and summary stats", async () => {
		...
		expect(res.status).toBe(200);
		const data = (await res.json()) as ComplianceSummaryResponse;
	...
	test("?view=control&id=AC-2 returns control detail", async () => {
		...
		expect(res.status).toBe(200);
		const data = (await res.json()) as ComplianceControlResponse;
...
describe("GET /api/compliance/score", () => {
	test("returns score with per-family breakdown", async () => {
		...
		expect(res.status).toBe(200);
		const data = (await res.json()) as ComplianceScoreResponse;
...
describe("POST /api/upload + GET/DELETE /api/evidence/*", () => {
	...
	test("uploads file with SHA-256 hash", async () => {
		...
		formData.append(
			"file",
			new File([testContent], testFilename, { type: "text/plain" }),
		);
