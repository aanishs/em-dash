...
async function normalize(tool: string, fixturePath: string): Promise<any[]> {
  const input = fs.readFileSync(fixturePath, 'utf8');
  const proc = Bun.spawn([path.join(BIN, 'comply-orchestrate'), 'normalize-test', tool], {
    cwd: ROOT,
...
describe('normalizeTool: prowler', () => {
  test('normalizes PASS and FAIL findings', async () => {
    const findings = await normalize('prowler', path.join(FIXTURES, 'prowler-sample.json'));
    expect(findings).toHaveLength(2);

    const pass = findings.find((f: any) => f.result === 'PASS');
    const fail = findings.find((f: any) => f.result === 'FAIL');
  ...
  test('resolves prowler check IDs to NIST controls', async () => {
    ...
    const withControls = findings.filter((f: any) => f.control_ids.length > 0);
    expect(withControls.length).toBeGreaterThanOrEqual(1);
  });

  test('handles empty input', async () => {
    const proc = Bun.spawn([path.join(BIN, 'comply-orchestrate'), 'normalize-test', 'prowler'], {
      cwd: ROOT,
      stdin: new Blob(['[]']),
      stdout: 'pipe',
      stderr: 'pipe',
...
describe('normalizeTool: checkov', () => {
  test('separates passed_checks from failed_checks', async () => {
    const findings = await normalize('checkov', path.join(FIXTURES, 'checkov-sample.json'));
    expect(findings).toHaveLength(2);
    const pass = findings.find((f: any) => f.result === 'PASS');
    const fail = findings.find((f: any) => f.result === 'FAIL');
    expect(pass).toBeDefined();
  ...
  test('handles missing results key', async () => {
    const proc = Bun.spawn([path.join(BIN, 'comply-orchestrate'), 'normalize-test', 'checkov'], {
      cwd: ROOT,
      stdin: new Blob(['{}']),
      stdout: 'pipe',
      stderr: 'pipe',
...
describe('normalizeTool: trivy', () => {
  test('extracts Vulnerabilities', async () => {
    const findings = await normalize('trivy', path.join(FIXTURES, 'trivy-sample.json'));
    const vulns = findings.filter((f: any) => f.check_id === 'CVE-2024-1234');
  ...
  test('extracts Misconfigurations with PASS/FINDING status', async () => {
    const findings = await normalize('trivy', path.join(FIXTURES, 'trivy-sample.json'));
    const misconfigs = findings.filter((f: any) => f.check_id.startsWith('AVD-'));
    expect(misconfigs).toHaveLength(2);
    expect(misconfigs.find((f: any) => f.result === 'FINDING')).toBeDefined();
    expect(misconfigs.find((f: any) => f.result === 'PASS')).toBeDefined();
  });

  test('extracts Secrets with CRITICAL severity', async () => {
    const findings = await normalize('trivy', path.join(FIXTURES, 'trivy-sample.json'));
    const secrets = findings.filter((f: any) => f.check_id === 'secret-detection');
  ...
  test('handles empty Results array', async () => {
    const proc = Bun.spawn([path.join(BIN, 'comply-orchestrate'), 'normalize-test', 'trivy'], {
      cwd: ROOT,
...
describe('normalizeTool: semgrep', () => {
  test('extracts check_id, path:line resource, severity', async () => {
    const findings = await normalize('semgrep', path.join(FIXTURES, 'semgrep-sample.json'));
    expect(findings).toHaveLength(2);
  ...
  test('handles empty results', async () => {
    const proc = Bun.spawn([path.join(BIN, 'comply-orchestrate'), 'normalize-test', 'semgrep'], {
      cwd: ROOT,
      stdin: new Blob(['{"results": []}']),
      stdout: 'pipe',
      stderr: 'pipe',
...
describe('normalizeTool: kube-bench', () => {
  test('extracts nested Controls > tests > results', async () => {
    const findings = await normalize('kube-bench', path.join(FIXTURES, 'kube-bench-sample.json'));
  ...
  test('maps scored=true to HIGH severity', async () => {
    const findings = await normalize('kube-bench', path.join(FIXTURES, 'kube-bench-sample.json'));
    const scored = findings.find((f: any) => f.check_id === '1.1.1');
    const unscored = findings.find((f: any) => f.check_id === '1.1.3');
...
describe('normalizeTool: default/unknown', () => {
  test('iterates array input', async () => {
    const proc = Bun.spawn([path.join(BIN, 'comply-orchestrate'), 'normalize-test', 'unknown-tool'], {
      cwd: ROOT,
  ...
  test('iterates { findings: [...] } wrapper', async () => {
    const proc = Bun.spawn([path.join(BIN, 'comply-orchestrate'), 'normalize-test', 'custom'], {
      cwd: ROOT,
