...
import { matchGlob, selectTests, E2E_TOUCHFILES, LLM_JUDGE_TOUCHFILES, GLOBAL_TOUCHFILES } from './helpers/touchfiles';

describe('matchGlob', () => {
  test('matches exact file', () => {
    expect(matchGlob('scripts/gen-skill-docs.ts', 'scripts/gen-skill-docs.ts')).toBe(true);
  });

  test('matches single wildcard', () => {
    expect(matchGlob('skills/hipaa-scan/SKILL.md', 'skills/*/SKILL.md')).toBe(true);
    expect(matchGlob('skills/hipaa-scan/SKILL.md.tmpl', 'skills/*/SKILL.md')).toBe(false);
  });

  test('matches globstar', () => {
    expect(matchGlob('skills/hipaa-scan/SKILL.md', 'skills/**')).toBe(true);
    expect(matchGlob('skills/hipaa-scan/nested/deep.ts', 'skills/**')).toBe(true);
...
describe('touchfile maps are well-formed', () => {
  ...
  test('all touchfile values are non-empty arrays', () => {
    for (const [name, patterns] of Object.entries(E2E_TOUCHFILES)) {
      expect(patterns.length).toBeGreaterThan(0);
    }
    for (const [name, patterns] of Object.entries(LLM_JUDGE_TOUCHFILES)) {
      expect(patterns.length).toBeGreaterThan(0);
    }
  });
});
