...
interface ClaudeEvent {
	type: string;
	message?: { content: ClaudeContentItem[] };
	is_error?: boolean;
	subtype?: string;
	result?: string;
	num_turns?: number;
...
export function parseNDJSON(lines: string[]): ParsedNDJSON {
	...
	let resultLine: ClaudeEvent | null = null;
	let turnCount = 0;
	let toolCallCount = 0;
...
export async function runSkillTest(options: {
	...
	if (complianceErrors.length > 0 || exitReason !== "success") {
		try {
			const failureDir =
				runDir || path.join(workingDirectory, ".em-dash", "test-transcripts");
