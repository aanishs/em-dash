...
const CODE_CHECKS: Check[] = [
	{
		...
		description:
			"Direct sensitive data identifiers in code (names, SSN, MRN, DOB, insurance IDs)",
		type: "code_grep",
		provider: "code",
		pattern:
			"patient.*name|ssn|social.security|medical.record|mrn|date.of.birth|dob|insurance.id",
	...
	},
	{
		...
		category: "sensitive_data",
		description:
			"Health-specific data fields (diagnosis, medications, lab results, clinical notes)",
		type: "code_grep",
		provider: "code",
		pattern:
			"diagnosis|icd.?10|medication|prescription|lab.result|clinical.note",
		severity_default: "INFO",
	},
	{
	...
	},
	{
		...
		description:
			"Sensitive data stored in localStorage, sessionStorage, cookies, or URL params",
		type: "code_grep",
		provider: "code",
		pattern:
			"localStorage\\.set.*patient|sessionStorage.*ssn|cookie.*phi|\\?.*ssn=",
		severity_default: "CRITICAL",
	},
	{
		id: "rbac-existence",
	...
	},
	{
		...
		category: "access_control",
		description:
			"Least-privilege IAM — no wildcard actions or hardcoded credentials in code",
		type: "code_grep",
		provider: "code",
	...
	},
	{
		...
		description:
			"Database connections use SSL — sslmode=disable is a critical finding",
		type: "code_grep",
		provider: "code",
...
const AWS_CHECKS: Check[] = [
	{
		...
		category: "authentication",
		description:
			"IAM account password policy configured with strong requirements",
		type: "cloud_cli",
		provider: "aws",
	...
	},
	{
		...
		provider: "aws",
		command:
			"aws iam generate-credential-report && aws iam get-credential-report --output json",
		severity_default: "HIGH",
	},
	{
		id: "aws-iam-wildcard",
		category: "access_control",
		description:
			"No IAM policies with Action:* and Resource:* (least privilege)",
		type: "cloud_cli",
		provider: "aws",
	...
	},
	{
		...
		category: "audit",
		description:
			"CloudTrail enabled, multi-region, with log file validation and KMS encryption",
		type: "cloud_cli",
		provider: "aws",
	...
	},
	{
		...
		provider: "aws",
		command:
			'aws logs describe-log-groups --query "logGroups[?!retentionInDays]" --output json',
		severity_default: "MEDIUM",
	},
	{
	...
	},
	{
		...
		category: "access_control",
		description:
			"S3 public access block configured at account and bucket level",
		type: "cloud_cli",
		provider: "aws",
	...
	},
	{
		...
		provider: "aws",
		command:
			'aws rds describe-db-instances --query "DBInstances[*].{ID:DBInstanceIdentifier,Encrypted:StorageEncrypted}" --output json',
		severity_default: "HIGH",
	},
	{
		id: "aws-kms-rotation",
	...
	},
	{
		...
		category: "encryption",
		description: "Azure Key Vault keys have rotation policy configured",
		type: "cloud_cli",
		provider: "azure",
		command:
			'az keyvault key list --vault-name VAULT --query "[].{name:name,rotationPolicy:rotationPolicy}" -o json',
		severity_default: "MEDIUM",
	},
	{
	...
	},
	{
		...
		provider: "aws",
		command:
			'aws secretsmanager list-secrets --query "SecretList[*].{Name:Name,Rotation:RotationEnabled}" --output json',
		severity_default: "MEDIUM",
	},
	{
		id: "aws-lambda-vpc",
		...
		provider: "aws",
		command:
			'aws lambda list-functions --query "Functions[*].{Name:FunctionName,VPC:VpcConfig}" --output json',
		severity_default: "MEDIUM",
	},
	{
		id: "aws-lambda-kms",
		category: "encryption",
		description:
			"Lambda environment variables encrypted with KMS (not default key)",
		type: "cloud_cli",
		provider: "aws",
		command:
			'aws lambda list-functions --query "Functions[*].{Name:FunctionName,KMS:KMSKeyArn}" --output json',
		severity_default: "MEDIUM",
	},
	{
...
const REGO_CHECKS: Check[] = [
	...
	{
		id: "rego-aws-sns-encryption",
		category: "encryption",
		description: "SNS topics must use KMS encryption",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-gcp-cloudsql-encryption",
		category: "encryption",
		description: "Cloud SQL instances must use customer-managed encryption key",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-gcp-gcs-encryption",
		category: "encryption",
		description: "GCS buckets must have customer-managed encryption",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-aws-sqs-encryption",
		category: "encryption",
		description: "SQS queues must use KMS encryption",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-azure-storage-https",
		category: "encryption",
	...
	{
		id: "rego-azure-disk-encryption",
		category: "encryption",
		description:
			"Azure managed disks must be encrypted with customer-managed key",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-azure-keyvault-softdelete",
		category: "encryption",
		description: "Azure Key Vault must have soft delete retention >= 7 days",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-gcp-bigquery-encryption",
		category: "encryption",
		description: "BigQuery datasets must use customer-managed encryption",
		type: "rego",
		severity_default: "MEDIUM",
	},
	...
	{
		id: "rego-aws-rds-backup",
		category: "data_protection",
		description: "RDS instances must have automated backups enabled",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-aws-rds-backup-retention",
		category: "data_protection",
		description: "RDS backup retention must be >= 35 days",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-aws-s3-versioning",
		category: "data_protection",
	...
	{
		id: "rego-aws-efs-backup",
		category: "data_protection",
		description:
			"EFS file systems must have lifecycle/backup policy configured",
		type: "rego",
		severity_default: "LOW",
	},
	{
		id: "rego-gcp-cloudsql-backup",
		category: "data_protection",
	...
	{
		id: "rego-azure-sql-backup",
		category: "data_protection",
		description: "Azure SQL databases must have long-term retention policy",
		type: "rego",
		severity_default: "MEDIUM",
	},
	...
	{
		id: "rego-aws-sg-rule-open",
		category: "transmission",
		description:
			"Security group rules must not allow 0.0.0.0/0 on sensitive ports",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		...
		category: "transmission",
		description:
			"Security groups must not allow 0.0.0.0/0 ingress on sensitive port ranges",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-aws-lb-https",
		category: "transmission",
		description: "ALB/NLB listeners must use HTTPS or TLS protocol",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-aws-rds-sg-open",
		category: "transmission",
		description: "RDS instances must have parameter group enforcing SSL",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-gcp-sql-authorized-networks",
		category: "transmission",
		description: "Cloud SQL instances must require SSL for connections",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-gcp-sql-public-ip",
		category: "transmission",
		description: "Cloud SQL instances must not have public IPv4 enabled",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-gcp-firewall-open",
		category: "transmission",
		description:
	...
	{
		id: "rego-azure-nsg-open",
		category: "transmission",
		description: "Azure NSG rules must not allow inbound * on sensitive ports",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-azure-appservice-https",
		category: "transmission",
		description: "Azure App Service must enforce HTTPS-only",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-gcp-cloudrun-public",
		category: "transmission",
		description:
			"Cloud Run must not allow unauthenticated (allUsers) invocations",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-docker-expose-all",
		category: "transmission",
		description: "Dockerfiles must not expose sensitive ports directly",
		type: "rego",
		severity_default: "MEDIUM",
	},
	...
	{
		id: "rego-aws-cloudtrail-multiregion",
		category: "audit",
		description: "CloudTrail must be configured as multi-region",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-aws-cloudtrail-logvalidation",
		category: "audit",
	...
	{
		id: "rego-aws-cloudtrail-encryption",
		category: "audit",
		description: "CloudTrail must be encrypted with KMS",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-aws-vpc-flowlogs",
		category: "audit",
	...
	{
		id: "rego-aws-cloudwatch-retention",
		category: "audit",
		description: "CloudWatch log groups must have retention >= 365 days",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-aws-cloudwatch-no-retention",
		category: "audit",
		description: "CloudWatch log groups must have a retention policy set",
		type: "rego",
		severity_default: "LOW",
	},
	{
		id: "rego-gcp-audit-logging",
		category: "audit",
		description:
			"GCP audit configs must not exempt members from DATA_READ logging",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-azure-monitor-logprofile",
		category: "audit",
		description: "Azure log profiles must have retention policy configured",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-azure-monitor-diagnostic",
		category: "audit",
		description: "Azure diagnostic settings must have retention enabled",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-gcp-logging-sink",
		category: "audit",
		description:
			"GCP log sinks must have a filter targeting audit-relevant log types",
		type: "rego",
		severity_default: "LOW",
	},
	...
	{
		id: "rego-aws-iam-wildcard-action",
		category: "access_control",
		description:
			"IAM policies must not grant wildcard Action and Resource (string form)",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-aws-iam-wildcard-resource",
		category: "access_control",
		description:
			"IAM policies must not grant wildcard Action and Resource (array form)",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-aws-iam-admin-policy",
		category: "access_control",
		description: "IAM role inline policies must not use wildcard actions",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-gcp-iam-primitive-role",
		category: "access_control",
		description: "GCP service accounts must not have roles/owner",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-gcp-iam-alluser-binding",
		category: "access_control",
		description:
			"GCP service accounts must not have roles/editor — use granular roles",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-azure-role-subscription-scope",
		category: "access_control",
		description: "Azure Contributor must not be assigned at subscription scope",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-azure-role-wildcard-scope",
		category: "access_control",
		description: "Azure Owner must not be assigned at subscription scope",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-gcp-sa-key-rotation",
		category: "access_control",
		description:
	...
	{
		id: "rego-gcp-iam-allauthenticated",
		category: "access_control",
		description:
			"GCP IAM bindings must not grant access to allUsers or allAuthenticatedUsers",
		type: "rego",
		severity_default: "CRITICAL",
	},
	{
		id: "rego-azure-role-wildcard-actions",
		category: "access_control",
		description: "Azure custom role definitions must not use wildcard actions",
		type: "rego",
		severity_default: "HIGH",
	},
	...
	{
		id: "rego-aws-provider-hardcoded-key",
		category: "secrets",
		description: "AWS provider must not have hardcoded access_key",
		type: "rego",
		severity_default: "CRITICAL",
	},
	{
		id: "rego-aws-provider-hardcoded-secret",
		category: "secrets",
		description: "AWS provider must not have hardcoded secret_key",
		type: "rego",
		severity_default: "CRITICAL",
	},
	{
		id: "rego-tf-sensitive-default",
		category: "secrets",
		description: "Terraform password variables must not have default values",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-tf-secret-var-default",
		category: "secrets",
		description:
			"Terraform secret/key/token variables must not have default values",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-k8s-secret-in-env",
		category: "secrets",
		description:
			"K8s env vars with secret names must use secretKeyRef, not inline values",
		type: "rego",
		severity_default: "HIGH",
	},
	...
	{
		id: "rego-docker-nonroot-user",
		category: "compute",
		description: "Dockerfiles must not set USER root",
		type: "rego",
		severity_default: "HIGH",
	},
	{
		id: "rego-docker-latest-tag",
		category: "compute",
		description:
			"Dockerfiles must not use :latest tag — pin to specific version",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-docker-add-url",
		category: "compute",
		description:
			"Dockerfiles must not use ADD with remote URLs — use COPY + curl",
		type: "rego",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-compose-readonly",
		category: "compute",
		description: "Docker Compose services must not run in privileged mode",
		type: "rego",
		severity_default: "CRITICAL",
	},
	...
	{
		id: "rego-k8s-deploy-nonroot",
		category: "compute",
		description: "Deployment containers must not run as root (UID 0)",
		type: "rego",
		provider: "k8s",
		severity_default: "HIGH",
	},
	{
		id: "rego-k8s-deploy-seccontext",
		category: "compute",
		description:
			"Deployment containers must set runAsNonRoot in securityContext",
		type: "rego",
		provider: "k8s",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-k8s-deploy-nonprivileged",
		category: "compute",
		description: "Deployment containers must not run in privileged mode",
		type: "rego",
		provider: "k8s",
		severity_default: "CRITICAL",
	},
	{
		id: "rego-k8s-namespace-netpolicy",
		category: "compute",
		description:
			"Namespaces labeled for sensitive data must have NetworkPolicy",
		type: "rego",
		provider: "k8s",
		severity_default: "HIGH",
	},
	{
		id: "rego-k8s-deploy-approved-registry",
		category: "compute",
		description:
			"Deployment containers must use images from approved registries",
		type: "rego",
		provider: "k8s",
		severity_default: "MEDIUM",
	},
	{
		id: "rego-k8s-deploy-secret-env",
		category: "secrets",
		description: "Deployment env vars with secret names must use secretKeyRef",
		type: "rego",
		provider: "k8s",
		severity_default: "HIGH",
	},
	{
		id: "rego-k8s-statefulset-nonroot",
		category: "compute",
		description: "StatefulSet containers must not run in privileged mode",
		type: "rego",
		provider: "k8s",
		severity_default: "CRITICAL",
	},
...
const TOOL_INTEGRATION_CHECKS: Check[] = [
	{
		...
		provider: "aws",
		command:
			"prowler --compliance hipaa --output-modes json --output-directory /tmp/prowler-out",
		severity_default: "HIGH",
	},
	{
		id: "tool-checkov",
	...
	},
	{
		...
		type: "tool_integration",
		command:
			"kics scan -p . --type terraform,kubernetes,docker,cloudformation -o /tmp/kics-out --report-formats json",
		severity_default: "HIGH",
	},
	{
		id: "tool-trivy",
	...
	},
	{
		...
		type: "tool_integration",
		command:
			"python3 -m ScoutSuite --provider aws --report-dir /tmp/scoutsuite-out --result-format json --no-browser",
		severity_default: "HIGH",
	},
	{
		id: "tool-lynis",
...
const POLICY_DOC_CHECKS: Check[] = [
	{
		...
		provider: "code",
		pattern:
			"incident.response.plan|incident.response.procedure|security.incident",
		severity_default: "MEDIUM",
	},
	{
		...
		provider: "code",
		pattern:
			"information.security.policy|security.policy.statement|hipaa.security.policy",
		severity_default: "MEDIUM",
	},
	{
		...
		provider: "code",
		pattern:
			"access.control.policy|authorization.policy|least.privilege.policy",
		severity_default: "MEDIUM",
	},
	{
		...
		provider: "code",
		pattern:
			"contingency.plan|disaster.recovery|business.continuity|bcp.plan|dr.plan",
		severity_default: "MEDIUM",
	},
	{
	...
	},
	{
		...
		provider: "code",
		pattern:
			"training.record|security.awareness|training.completion|training.program",
		severity_default: "MEDIUM",
	},
	{
		...
		provider: "code",
		pattern:
			"media.protection|data.disposal|media.sanitization|device.disposal",
		severity_default: "MEDIUM",
	},
	{
		...
		provider: "code",
		pattern:
			"contingency.test|dr.test|disaster.recovery.test|failover.test|backup.test",
		severity_default: "LOW",
	},
	{
		...
		provider: "code",
		pattern:
			"security.assessment|audit.report|compliance.report|pentest.report|vulnerability.scan",
		severity_default: "LOW",
	},
	{
		...
		provider: "code",
		pattern:
			"continuous.monitoring|monitoring.policy|siem.config|log.monitoring|alert.config",
		severity_default: "LOW",
	},
];
